...
def dropbox_link(
    ...
    *,
    ...
    dl: int = 0,
    access_token=DFLT_ACCESS_TOKEN,
) -> str:
    """
    ...
    Parameters:
      ...
      access_token (str): The key to look for in os.environ or the literal access token if not found.
    ...
    shared_url = create_or_get_shared_link(dbx, dbx_file_path)
...
class DropboxPersister(Persister):
    ...
    def __init__(
        ...
        self._con = Dropbox(access_token, **connection_kwargs)
...
class DropboxLinkReaderWithToken(ReadOnlyMixin, DropboxPersister):
    def __init__(self, url, access_token=DFLT_ACCESS_TOKEN):
        self._con = Dropbox(access_token)
        self.url = url
